...
beforeEach(() => {
  vi.clearAllMocks();
  vi.mocked(convexReact.useQuery).mockReturnValue(MOCK_FOLLOWERS as any); // eslint-disable-line @typescript-eslint/no-explicit-any
  vi.mocked(convexReact.useMutation).mockImplementation(
    () => vi.fn().mockResolvedValue(null) as any, // eslint-disable-line @typescript-eslint/no-explicit-any
  );
});
...
describe("FollowerManagementPanel", () => {
  ...
  it("shows loading state while query is undefined", () => {
    vi.mocked(convexReact.useQuery).mockReturnValue(undefined as any); // eslint-disable-line @typescript-eslint/no-explicit-any
    render(<FollowerManagementPanel token="test-token" />);
  ...
  it("calls banUser mutation on confirmation", async () => {
    ...
    vi.mocked(convexReact.useMutation).mockImplementation(
      () => mockBan as any, // eslint-disable-line @typescript-eslint/no-explicit-any
    );
  ...
  it("shows Issue reset confirmation and calls mutation", async () => {
    ...
    vi.mocked(convexReact.useMutation).mockImplementation(
      () => mockReset as any, // eslint-disable-line @typescript-eslint/no-explicit-any
    );
