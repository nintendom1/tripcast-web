      ...
      <div className="flex flex-col gap-3 overflow-y-auto h-full">
        ...
        {isCreating && (
          <div className="px-4 pb-2 border-b border-slate-100">
            <TravelerCreateForm token={token} onSuccess={() => setIsCreating(false)} />
          </div>
        )}
...
function TravelerCreateForm({ token, onSuccess }: { token: string; onSuccess: () => void }) {
...
type CrewTab = "propose" | "mine" | "public";
...
function SupportCrewChallengePanel({
  ...
  const [tab, setTab] = useState<CrewTab>("public");
  const [selectedChallenge, setSelectedChallenge] = useState<Challenge | null>(null);
  const [isDetailOpen, setIsDetailOpen] = useState(false);
  ...
  function openDetail(challenge: Challenge, isOwn: boolean) {
    setSelectedChallenge(challenge);
    setIsDetailOpen(true);
  }
    ...
    <>
      <div className="flex flex-col gap-3 overflow-y-auto h-full">
        ...
        <div className="flex gap-1 px-4 pt-2 border-b border-slate-100 pb-2">
          {([
            { value: "propose" as CrewTab, label: "Propose" },
            { value: "mine" as CrewTab, label: `Mine (${mine.length})` },
            { value: "public" as CrewTab, label: "Active" },
          ...
          <div className="flex flex-col gap-2 px-4 pb-4">
            ...
            {publicChallenges.map((c) => (
              <ChallengeCard
                ...
                onClick={() => openDetail(c, mineIds.has(c._id))}
              />
            ))}
          </div>
        )}
...
export default function ChallengePanel({ open, token, role, sessionId, userId, onClose, onStartChallenge }: Props) {
  return (
    <AnimatePresence>
      {open && (
        <motion.div
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto">
            {role === "traveler" ? (
              ...
              <SupportCrewChallengePanel token={token} userId={userId} />
            )}
          </div>
        </motion.div>
      )}
