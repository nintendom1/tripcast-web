...
## Objectives
- **Follow Visited Trail:** Adjust replay to strictly follow completed story pins and live trails.
- **Exclude Missions:** Remove all missions (proposed, visible, planned, in-progress, completed, or dropped) and route votes from the replay sequence.
...
- **Dwell on Stories:** Ensure story pins dwell for the POI card to be read.
- **Fix Notes:** Ensure POI cards correctly display the note body for story events.
...
- Updated `buildReplayPins` to filter journal events strictly for the `story` type.
